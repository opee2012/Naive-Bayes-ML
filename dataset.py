  ...
  """
    ...
    """
        ...
        if cleanedType == "cat":
            ...
            # then treat like a range of ordinal values
                ...
                raise Exception(
                    "Program could not understand ordinal attribute value specification for attribute " + str(
                        cleanedName) + ".")
    ...
    """
        if (idx < 0) or (idx >= len(self.instances)):
            raise Exception("Requesting instance " + str(idx) + ", but there are only " + str(
                len(self.instances)) + " instances in the dataset.")

        if not attributeName in self.instances[idx]:
            raise Exception("Requested attribute " + str(attributeName) + " for instance " + str(
                idx) + "; however, no such attribute exists.")
...
def unitTest():
  ...
  """
    ...
    if len(sys.argv) > 2:
        ...
        print("Found", len(subsetOfInstances), "out of", len(ds.instances), "instances matching criteria",
              selectionCriteria)
        print()
